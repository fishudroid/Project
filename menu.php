...
$product = $conn->query("SELECT *, price - ((price * sale) / 100) as sale_price FROM product ORDER BY id DESC");
$category = '';
if (!empty($_GET['cat'])) {
  $catId = $_GET['cat'];
  $product = $conn->query("SELECT *,price-(price*sale)/100 as sale_price FROM product  WHERE category_id = $catId Order By id DESC");
  ...
  <div class="container">
    ...
    <ul class="filters_menu">
      ...
      <?php while ($cat = $cats->fetch_object()) : ?>
        <li class="<?php echo $cat->id == $catId ? 'active' : ''; ?>"><a href="menu.php?cat=<?php echo $cat->id; ?>"><?php echo $cat->name; ?></a></li>
      <?php endwhile; ?>
    </ul>

    <div class="filters-content">
      <div class="row grid">
        <?php while ($sp = $product->fetch_object()) : ?>
