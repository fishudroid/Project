          ...
          <tbody>
            <?php while ($cat = $data->fetch_object()) : ?>
              <tr>
                ...
                <td><?php echo $cat->$status == 0 ? 'Hiện' : 'Ẩn'; ?></td>
                <td class="text-right">
                  <a href="category-edit.php?id=<?php echo $prod->id; ?>" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i> Chỉnh sửa</a>
                  <a onclick="return confirm('Bạn chắc chắn xoá danh mục này không ?')" href="category-delete.php?id=<?php echo $prod->id; ?>" class="btn btn-sm btn-primary"><i class="fa fa-trash"></i> Xoá</a>
                </td>
              </tr>
            <?php endwhile; ?>
          </tbody>
